...
// Obtener usuarios (single-site: no usamos tabla sedes)
export const OBRS_UsuariosPilatesPorSede_CTS = async (req, res) => {
  try {
    // Nota: si se envía `sede_id` en la query se ignora (compatibilidad)
...
export const CR_UsuarioPilates_CTS = async (req, res) => {
  try {
    ...
    // Validaciones de campos obligatorios (sede_id ya no es obligatorio)
    if (!nombre || !apellido || !email || !password || !rol) {
      return res.status(400).json({
        error: "Faltan datos obligatorios: nombre, apellido, email, password y rol son requeridos"
      });
    }
...
export const UR_UsuarioPilates_CTS = async (req, res) => {
  try {
    ...
    const datosActualizados = {
      ...
      estado,
      rol,
      sede_id: sede_id !== undefined ? sede_id : usuarioExistente.sede_id
    };
