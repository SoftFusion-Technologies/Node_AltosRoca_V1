...
// Obtiene los horarios de Pilates por sede, instructor y hora, junto con los alumnos inscriptos y sus detalles (sin agrupar por L-M-V / M-J).
export const ESP_OBRS_HorarioClientesPilates_CTS = async (req, res) => {
  try {
    // Validaciones de parámetros recibidos
    const { instructorId, hhmm } = req.query;
  ...
  const horarioWhere = {};
  // Nota: ya no filtramos por sede (single-site). Mantener filtro por instructor y hora si vienen.
  ...
  if (hhmm) horarioWhere.hora_inicio = toTimeWithSeconds(hhmm);
    ...
    // Mapas para referenciar horarios individuales
      ...
      include: [
        {
          ...
          as: "cliente",
          attributes: ["id", "nombre", "telefono", "estado", "fecha_inicio", "fecha_fin", "observaciones"],
          required: true,
        },
        {
    ...
    // Agrupación de inscripciones por horario específico
    const planHorarioMap = new Map();
    ...
    for (const inscripcion of inscripciones) {
      ...
      if (!horarioIdSet.has(horario.id)) {
        continue;
      }
      ...
      if (estadoLower === "plan" || estadoLower === "renovacion programada") {
        const currentPlans = planHorarioMap.get(horario.id) ?? [];
        currentPlans.push(baseCliente);
        planHorarioMap.set(horario.id, currentPlans);
      }
    }

    // Helper para indicar el día asociado en la respuesta
    const planTypeByDay = (dayUpper) => dayUpper || "";
    ...
    for (const horario of horarios) {
      ...
      const { horarioKey, group, day } = keyInfo;
      ...
      const planEntries = planHorarioMap.get(horario.id) ?? [];
...
export const CR_ClientesPilates_CTS = async (req, res) => {
  try {
    const { nombre, telefono, estado, fecha_inicio, fecha_fin, observaciones } = req.body;
    ...
    console.log("Datos validados:", { nombre, telefono, estado, fecha_inicio, fecha_fin, observaciones });
...
export const UR_ClientesPilates_CTS = async (req, res) => {
  try {
    ...
    const { nombre, apellido, telefono, estado, fecha_inicio, fecha_fin, observaciones } =
      req.body;
...
// Obtener horarios disponibles con cupo e inscriptos por sede para el apartado de ventas, así sepan cuántos lugares quedan en cada horario y grupo
...
export const ESP_OBRS_HorariosDisponibles_CTS = async (req, res) => {
  try {
    // Este endpoint ya no usa SedeModel ni múltiples sedes.
    // Se acepta un parámetro opcional `cupo` para especificar cupo por clase.
    // Si no se provee, se usa 6 como valor por defecto.
    const { cupo } = req.query;
    const sedeCupo = Number.isFinite(Number(cupo)) ? Number(cupo) : 6;
  ...
  const allowedGroups = new Set(["LMV", "MJ"]);
    ...
    // Obtener todos los horarios (sin filtrar por sede)
    const horarios = await HorariosPilatesModel.findAll({
      attributes: ["id", "dia_semana", "hora_inicio"],
    });
