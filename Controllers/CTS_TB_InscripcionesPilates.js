...
export const CR_InscripcionesPilates_CTS = async (req, res) => {
  try {
    const { id_cliente, dia, horario, fecha_inscripcion } = req.body;

    // Validaciones básicas
    if (!id_cliente || !dia || !horario || !fecha_inscripcion) {
      return res.status(400).json({
        mensajeError: "id_cliente, dia, horario y fecha_inscripcion son requeridos",
      });
    }

    console.log("Datos recibidos para la inscripcion:", req.body);

    // Buscar el id del horario correspondiente (single-site: no filtramos por sede)
    const horarioResult = await db.query(
      `SELECT id 
       FROM horarios_pilates
    ...
    if (!horarioResult || horarioResult.length === 0) {
      return res.status(404).json({
        mensajeError: "No se encontró el horario indicado",
        debug: { dia, horario },
      });
    }
